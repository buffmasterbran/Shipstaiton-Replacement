...
async function apiCall(body: Record<string, any>) {
  const res = await fetch('/api/carriers/direct', {
    ...
    body: JSON.stringify({ carrier: 'fedex', ...body }),
  })
  return res.json()
}

export default function FedExDirectTab() {
  ...
  const [rateServiceCode, setRateServiceCode] = useState('FEDEX_GROUND')
  ...
  const fetchConfig = useCallback(async () => {
    try {
      ...
      const fedex: DirectConnectionConfig[] = data.connections?.fedex || []
      setConnections(fedex)
      const edits: typeof editState = {}
      const svcs: Record<string, string[]> = {}
      for (const c of fedex) {
  ...
  function applyConnectionsUpdate(fedexArr: DirectConnectionConfig[]) {
    setConnections(fedexArr)
    const edits: typeof editState = {}
    const svcs: Record<string, string[]> = {}
    for (const c of fedexArr) {
      edits[c.id] = { nickname: c.nickname, clientId: c.clientId, clientSecret: c.clientSecret, accountNumber: c.accountNumber, sandbox: c.sandbox }
      svcs[c.id] = c.enabledServices || []
    }
    setEditState(edits)
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      applyConnectionsUpdate(data.connections?.fedex || [])
  ...
  const handleTest = async (id: string) => {
    ...
    try {
      ...
      if (data.connections?.fedex) applyConnectionsUpdate(data.connections.fedex)
  ...
  const handleRateShop = async (connId: string) => {
    ...
    try {
      const codes = FEDEX_SERVICES.filter(s => s.domestic).map(s => s.code)
  ...
  const openLabel = (base64: string, format?: string) => {
    ...
    if (!win) return
    win.document.write(`<!DOCTYPE html><html><head><title>FedEx Test Label (4x6)</title><style>@page{size:4in 6in;margin:0}body{margin:0;display:flex;justify-content:center;align-items:center;background:#f3f4f6;min-height:100vh}img{width:4in;height:6in;object-fit:contain}@media print{body{background:white}}</style></head><body><img src="data:image/png;base64,${base64}"/></body></html>`)
    win.document.close()
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-purple-50 border border-purple-200 rounded-lg flex items-center justify-center">
            <span className="text-xs font-bold text-purple-800">FDX</span>
          </div>
          <div>
            <h3 className="text-lg font-semibold text-gray-900">FedEx Direct Connections</h3>
            <p className="text-sm text-gray-500">Connect directly to the FedEx REST API</p>
          </div>
        </div>
        <button onClick={() => setShowAddForm(true)} className="px-4 py-2 bg-purple-600 text-white rounded-lg text-sm font-medium hover:bg-purple-700 transition-colors flex items-center gap-2">
      ...
      {/* Add Form */}
      {showAddForm && (
        <div className="bg-white border-2 border-purple-300 rounded-xl p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h4 className="text-sm font-semibold text-gray-700 uppercase tracking-wide">New FedEx Connection</h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              ...
              <input type="text" value={addNickname} onChange={e => setAddNickname(e.target.value)} placeholder='e.g. "E-COM" or "Wholesale"' className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">API Key (Client ID)</label>
              <input type="text" value={addClientId} onChange={e => setAddClientId(e.target.value)} placeholder="From FedEx Developer Portal" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Secret Key (Client Secret)</label>
              <input type="password" value={addClientSecret} onChange={e => setAddClientSecret(e.target.value)} placeholder="••••••••" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">FedEx Account Number</label>
              <input type="text" value={addAccountNumber} onChange={e => setAddAccountNumber(e.target.value)} placeholder="e.g. 123456789" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500" />
            </div>
            <div className="flex items-end">
              <label className="flex items-center gap-2 cursor-pointer py-2">
                <input type="checkbox" checked={addSandbox} onChange={e => setAddSandbox(e.target.checked)} className="w-4 h-4 rounded border-gray-300 text-purple-600 focus:ring-purple-500" />
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button onClick={handleAdd} disabled={adding || !addClientId || !addClientSecret || !addAccountNumber} className="px-4 py-2 bg-purple-600 text-white rounded-lg text-sm font-medium hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
      ...
      {connections.length === 0 && !showAddForm && (
        <div className="bg-gray-50 border border-gray-200 border-dashed rounded-xl p-8 text-center">
          <p className="text-gray-500 text-sm">No FedEx Direct connections configured.</p>
          ...
          <div key={conn.id} className="bg-white border border-gray-200 rounded-xl overflow-hidden">
            <button onClick={() => setExpandedId(isExpanded ? null : conn.id)} className="w-full px-6 py-4 flex items-center justify-between hover:bg-gray-50 transition-colors text-left">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 bg-purple-50 border border-purple-200 rounded-lg flex items-center justify-center flex-shrink-0">
                  <span className="text-xs font-bold text-purple-800">FDX</span>
                </div>
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {savedServices.length > 0 && <span className="px-2 py-0.5 bg-purple-100 text-purple-800 text-xs font-semibold rounded-full">{savedServices.length} services</span>}
                  </div>
                  <div className="text-xs text-gray-500 mt-0.5">
                    Account: {conn.accountNumber}
                    {conn.sandbox && <span className="ml-2 text-purple-600 font-medium">(Sandbox)</span>}
              ...
              <div className="border-t border-gray-200 px-6 py-5 space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="md:col-span-2">
                      <label className="block text-xs font-medium text-gray-600 mb-1">Nickname</label>
                      <input type="text" value={edit.nickname} onChange={e => setEditState(p => ({ ...p, [conn.id]: { ...p[conn.id], nickname: e.target.value } }))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-600 mb-1">API Key (Client ID)</label>
                      <input type="text" value={edit.clientId} onChange={e => setEditState(p => ({ ...p, [conn.id]: { ...p[conn.id], clientId: e.target.value } }))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-600 mb-1">Secret Key (Client Secret)</label>
                      <input type="password" value={edit.clientSecret} onChange={e => setEditState(p => ({ ...p, [conn.id]: { ...p[conn.id], clientSecret: e.target.value } }))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-600 mb-1">FedEx Account Number</label>
                      <input type="text" value={edit.accountNumber} onChange={e => setEditState(p => ({ ...p, [conn.id]: { ...p[conn.id], accountNumber: e.target.value } }))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500" />
                    </div>
                    <div className="flex items-end">
                      <label className="flex items-center gap-2 cursor-pointer py-2">
                        <input type="checkbox" checked={edit.sandbox} onChange={e => setEditState(p => ({ ...p, [conn.id]: { ...p[conn.id], sandbox: e.target.checked } }))} className="w-4 h-4 rounded border-gray-300 text-purple-600 focus:ring-purple-500" />
                        <span className="text-sm text-gray-700">Sandbox / Testing Mode</span>
                      </label>
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  <button onClick={() => handleSave(conn.id)} disabled={state.saving || !edit.clientId || !edit.clientSecret || !edit.accountNumber} className="px-4 py-2 bg-purple-600 text-white rounded-lg text-sm font-medium hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                    {state.saving ? 'Saving...' : hasChanges ? 'Save Changes' : 'Save'}
                  </button>
                  <button onClick={() => handleTest(conn.id)} disabled={state.testing || !!hasChanges} className="px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                ...
                {state.testLabelResult && (
                  <div className={`border rounded-xl p-5 ${state.testLabelResult.success ? 'bg-purple-50 border-purple-200' : 'bg-red-50 border-red-200'}`}>
                    <p className={`font-semibold text-sm ${state.testLabelResult.success ? 'text-purple-800' : 'text-red-800'}`}>
                    ...
                    {state.testLabelResult.success && (
                      <div className="mt-2 flex items-center gap-4 text-xs">
                        ...
                        {state.testLabelResult.labelBase64 && (
                          <button onClick={() => openLabel(state.testLabelResult!.labelBase64!, state.testLabelResult!.labelFormat)} className="px-3 py-1 bg-purple-600 text-white rounded text-xs font-medium hover:bg-purple-700">View 4x6 Label</button>
                        )}
                      </div>
                    )}
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => handleSaveServices(conn.id)} disabled={state.savingServices || !servicesChanged} className="px-3 py-1.5 bg-purple-600 text-white rounded-lg text-xs font-medium hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                    {FEDEX_SERVICES.map(svc => (
                      <label key={svc.code} className={`flex items-center gap-2 px-3 py-2 rounded-lg border-2 cursor-pointer text-sm transition-colors ${connSelectedServices.includes(svc.code) ? 'border-purple-400 bg-purple-50' : 'border-gray-200 hover:border-gray-300'}`}>
                        <input type="checkbox" checked={connSelectedServices.includes(svc.code)} onChange={() => toggleService(conn.id, svc.code)} className="w-4 h-4 text-purple-600 rounded border-gray-300 focus:ring-purple-500" />
                  ...
                  {isSuiteOpen && (
                    <div className="border-t border-gray-200 p-5 space-y-6">
                      ...
                      <div className="space-y-3">
                        ...
                        <button onClick={() => handleValidateAddress(conn.id)} disabled={avLoading || conn.status !== 'connected'} className="px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
                          <select value={rateServiceCode} onChange={e => setRateServiceCode(e.target.value)} className="px-2 py-1.5 border border-gray-300 rounded text-sm col-span-2">
                            {FEDEX_SERVICES.map(s => <option key={s.code} value={s.code}>{s.name}</option>)}
                          </select>
                        ...
                        <button onClick={() => handleGetRate(conn.id)} disabled={rateLoading || conn.status !== 'connected'} className="px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
