...
/** Delete queue item (admin only). Releases that batch back so orders can be re-processed from Bulk Orders. */
export async function DELETE(
  ...
  try {
    const { id } = await params
    await prisma.bulkQueueItem.delete({
      where: { id },
    })
    return NextResponse.json({ success: true, message: 'Released from queue' })
  } catch (error: any) {
    if (error?.code === 'P2025') {
      return NextResponse.json({ error: 'Queue item not found' }, { status: 404 })
    }
    console.error('Error deleting bulk queue item:', error)
    return NextResponse.json(
