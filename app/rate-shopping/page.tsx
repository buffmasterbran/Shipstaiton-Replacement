...
export default function RateShoppingPage() {
  ...
  async function fetchRateShoppers() {
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Failed to fetch rate shoppers')
      }
  ...
  async function fetchCarriers() {
    try {
      ...
      // Fetch carriers with their services included
  ...
  function addService(carrier: Carrier, service: { service_code: string; name: string }) {
    ...
    if (!exists) {
      setFormServices([...formServices, newService])
    }
  }

  function removeService(carrierId: string, serviceCode: string) {
    setFormServices(
      formServices.filter((s) => !(s.carrierId === carrierId && s.serviceCode === serviceCode))
    )
    // If this was the preferred service, clear it
    if (formPreferredService === serviceCode) {
      setFormPreferredService('')
    }
  }
  ...
  async function handleSave() {
    if (!formName.trim()) {
      alert('Please enter a name for the rate shopper')
      return
    }
    ...
    try {
      ...
      const url = editingRateShopper
        ? `/api/rate-shoppers/${editingRateShopper.id}`
        : '/api/rate-shoppers'
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Failed to save rate shopper')
      }

      await fetchRateShoppers()
      closeModal()
    } catch (err: any) {
      console.error('Error saving rate shopper:', err)
  ...
  async function handleDelete(id: string) {
    if (!confirm('Are you sure you want to delete this rate shopper?')) {
      return
    }
    ...
    try {
      ...
      if (!response.ok) {
        const data = await response.json()
        throw new Error(data.error || 'Failed to delete rate shopper')
      }

      await fetchRateShoppers()
    } catch (err: any) {
      console.error('Error deleting rate shopper:', err)
      alert(err.message || 'Failed to delete rate shopper')
    }
  }
  ...
  // Get all available services from carriers
  const allServices = carriers.flatMap((carrier) =>
    (carrier.services || [])
      .filter((s) => s.domestic) // Only domestic for now
      .map((service) => ({
        carrier,
        service,
      }))
  ...
  return (
    <div className="p-8">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold">Rate Shopper</h1>
      ...
      {!loading && !error && rateShoppers.length === 0 && (
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-12 text-center">
          ...
          <p className="text-gray-500 mb-4">
            Create a rate shopper to automatically compare shipping services and select the best
            rate.
          </p>
          <button
            onClick={openCreateModal}
              ...
              <div className="p-4">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="mt-3">
                      <p className="text-xs text-gray-500 mb-2">
                        1. Compare these services ({rateShopper.services?.length || 0})
                      </p>
                      <div className="flex flex-wrap gap-2">
                        {(rateShopper.services || []).slice(0, 4).map((service, idx) => (
                          <span
                            key={idx}
                            className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-blue-50 border border-blue-200 text-blue-800 text-sm rounded"
                          >
                            <svg
                        ...
                        {(rateShopper.services?.length || 0) > 4 && (
                          <span className="px-2.5 py-1 bg-gray-100 text-gray-600 text-sm rounded">
                            +{(rateShopper.services?.length || 0) - 4} more
                          </span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="mt-3 text-sm text-gray-600">
                      ...
                      {rateShopper.transitTimeRestriction &&
                        rateShopper.transitTimeRestriction !== 'no_restriction' && (
                          <p className="text-xs text-gray-500 mt-1">
                            Transit time: {rateShopper.transitTimeRestriction.replace('_', ' ')}
                          </p>
                        )}
                      {rateShopper.preferenceEnabled && rateShopper.preferredServiceCode && (
                        <p className="text-xs text-gray-500 mt-1">
                          Prefer: {rateShopper.preferredServiceCode} if within{' '}
                          {rateShopper.preferenceType === 'dollar'
                            ? `$${rateShopper.preferenceValue}`
                            : `${rateShopper.preferenceValue}%`}
                        </p>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-2 ml-4">
                    {!rateShopper.isDefault && (
                      <button
                        onClick={() => handleSetDefault(rateShopper.id)}
              ...
              <div className="flex">
                ...
                <div className="flex-1 p-6 border-r border-gray-200 max-h-[70vh] overflow-y-auto">
                  ...
                  <div className="mb-6">
                    ...
                    <p className="text-xs text-gray-500 mb-2">
                      ShipStation will automatically choose the cheapest of these services
                    </p>
                    ...
                    {formServices.length > 0 && (
                      <div className="flex flex-wrap gap-2 mb-3">
                        {formServices.map((service, idx) => (
                          <span
                            key={idx}
                            className="inline-flex items-center gap-1.5 px-2.5 py-1.5 bg-blue-50 border border-blue-200 text-blue-800 text-sm rounded"
                          >
                            <svg
                              className="w-4 h-4 text-blue-500"
                              fill="currentColor"
                            ...
                            <span className="text-xs text-blue-600">({service.carrierName})</span>
                            <button
                              onClick={() => removeService(service.carrierId, service.serviceCode)}
                            ...
                            <div key={carrier.carrier_id}>
                              ...
                              {carrier.services && carrier.services.length > 0 ? (
                                carrier.services
                                  .filter((s) => s.domestic)
                                  .map((service) => {
                                    const isSelected = formServices.some(
                                      (s) =>
                                        s.carrierId === carrier.carrier_id &&
                                        s.serviceCode === service.service_code
                                    )
                                    return (
                                      <div
                                        ...
                                        onClick={() =>
                                          isSelected
                                            ? removeService(carrier.carrier_id, service.service_code)
                                            : addService(carrier, service)
                                        }
                                      >
                  ...
                  <div className="mb-6">
                    ...
                    <p className="text-xs text-gray-500 mb-2">
                      Restricts eligible services to those that deliver within specified time
                    </p>
                    <select
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    ...
                    <div className="flex items-center justify-between mb-2">
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          Override the service selection with "Service Preference"
                        </p>
                      </div>
                      <button
                        type="button"
                        ...
                        className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${
                          formPreferenceEnabled ? 'bg-blue-600' : 'bg-gray-200'
                        }`}
                      >
                        <span
                          className={`inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                            formPreferenceEnabled ? 'translate-x-5' : 'translate-x-0'
                          }`}
                        />
                      </button>
                    </div>
                      ...
                      <div className="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                        <div className="mb-4">
                          ...
                          >
                            ...
                            {formServices.map((service, idx) => (
                              <option key={idx} value={service.serviceCode}>
                                {service.serviceName} ({service.carrierName})
                              </option>
                            ))}
                          </select>
                        </div>
                        ...
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-2">
                            If the cheapest label is within
                          </label>
                          <div className="flex items-center gap-4">
                            <label className="flex items-center gap-2">
                              <input
                                ...
                                name="preferenceType"
                                value="dollar"
                                checked={formPreferenceType === 'dollar'}
                                onChange={() => setFormPreferenceType('dollar')}
                            ...
                            <label className="flex items-center gap-2">
                              <input
                                ...
                                name="preferenceType"
                                value="percentage"
                                checked={formPreferenceType === 'percentage'}
                                onChange={() => setFormPreferenceType('percentage')}
                                className="text-blue-600 focus:ring-blue-500"
                              />
                          ...
                          <div className="flex gap-2 mt-2">
                            <div className="flex-1 relative">
                              ...
                              <input
                                ...
                                min="0"
                                className={`w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${
                                  formPreferenceType === 'dollar' ? 'pl-7' : ''
                                }`}
                                placeholder={formPreferenceType === 'dollar' ? '0.00' : '0'}
                                value={formPreferenceValue || ''}
                                onChange={(e) =>
                                  setFormPreferenceValue(parseFloat(e.target.value) || 0)
                                }
                              />
                ...
                <div className="w-80 p-6 bg-gray-50">
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">
                    {formName || 'Untitled Rate Shopper'}
                  </h3>

                  <div className="space-y-4">
                    <div>
                      <p className="text-sm font-medium text-gray-700 mb-2">
                        1. Compare these services
                      </p>
                      <div className="flex flex-wrap gap-2">
                        {formServices.length === 0 ? (
                          ...
                          formServices.map((service, idx) => (
                            <span
                              key={idx}
                              className="inline-flex items-center gap-1 px-2 py-1 bg-blue-50 border border-blue-200 text-blue-800 text-xs rounded"
                            >
                              <svg
                    ...
                    <div>
                      ...
                      {formTransitTime !== 'no_restriction' && (
                        <p className="text-xs text-gray-500 mt-1">
                          Within {formTransitTime.replace('_', ' ')}
                        </p>
                      )}
                    </div>
                    ...
                    {formPreferenceEnabled && formPreferredService && (
                      <div>
                        ...
                        <p className="text-xs text-gray-500 mt-1">
                          Use{' '}
                          {formServices.find((s) => s.serviceCode === formPreferredService)
                            ?.serviceName || formPreferredService}{' '}
                          if within{' '}
                          {formPreferenceType === 'dollar'
                            ? `$${formPreferenceValue}`
                            : `${formPreferenceValue}%`}
